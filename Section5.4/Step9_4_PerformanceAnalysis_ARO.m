...
for i = 1:number_instance
    load(['DATA/DC' num2str(instance_type) '/Instance_Solution/result_dist' num2str(i) '.mat'],'target');
    
    for solution_approach_index = 1:2
        
        solution_approach = solution_type_collection{solution_approach_index};
        eval(['load(''DATA/DC' num2str(instance_type) '/WorstSample_ARO/Cost_' solution_approach num2str(i) '.mat'');']);
    ...
    p_var10 = [cost_aro_prob(quantile_index_aro+1,1); cost_dist_prob(quantile_index_dist+1,1) ] - target;
    ...
    p_var5 = [cost_aro_prob(quantile_index_aro+1,1); cost_dist_prob(quantile_index_dist+1,1)] - target;
...
eval(['save(''DATA/DC' num2str(instance_type) '/WorstCostAnalysis_ARO.mat'', ''P_*'');']);